...
class Salary:
    ...
    def pay_salary(self, emp, amount):
        if not isinstance(emp, Employee) or amount < 10000 or amount > 100000:
            raise ValueError("Invalid employee object or salary out of range")
    ...
    def get_salary_result(self, emp):
        if emp.emp_id in self.salary:
            if (80000 <= self.salary[emp.emp_id] <= 100000):
                return f"the salary of {emp} is {self.salary[emp.emp_id]} with {emp.tax_percentage} is an Extraordinary Employee "
            elif (60000 <= self.salary[emp.emp_id] < 80000):
                return f"the salary of {emp} is {self.salary[emp.emp_id]}  with {emp.tax_percentage} and is an Good employee"
            elif (30000 <= self.salary[emp.emp_id] < 60000):
                return f"the salary of {emp} is {self.salary[emp.emp_id]}  with {emp.tax_percentage} and is an Average employee"
            else:
...
def main():
    ...
    employee = Employee("pavan", "Kumar", 2, 40)
    salary.pay_salary(employee, 89000)
